...
search_space = {
    'kernel__k1__constant_value': (0.1, 100),
    'kernel__k2__length_scale': (1, 1000)
}

# Perform Bayesian optimization
bayes_search = BayesSearchCV(estimator=gpr, search_spaces=search_space, n_iter=50, cv=5, n_jobs=-1, scoring='neg_mean_squared_error', random_state=42)
bayes_search.fit(X_train_imputed, y_train)

# Print the best parameters found by BayesSearchCV
print(f"Best parameters: {bayes_search.best_params_}")
print(f"Best MSE: {-bayes_search.best_score_}")
